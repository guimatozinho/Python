...
class MainWindow(tkinter.Tk):
    ...
    def __init__(self):
        ...
        menuEditar.add_command(label="Importar XLS", underline=0, command=self.importar_db)
        menuEditar.add_command(label="Exportar XLS", underline=0, command=self.exportar_db)
        ...
        # Número do Código
        ttk.Label(self.mainframe, text='Código: ').grid(row=4, column=1, sticky=tkinter.E)
    ...
    def sobre(self, *ignore):
        messagebox.showinfo(message='Cadastro de Pacientes versão 0.1.1', title='Sobre')
    ...
    def importar_db(self, *ignore):
        ...
        options = {}
        options['filetypes'] = [('Arquivos XLS', '.xls'), ('Todos Arquivos', '.*')]
        options['initialfile'] = 'patients.xls'
        fileName = askopenfilename(**options)
        if not fileName:
            return
        
        try:
            tree = xls.etree.ElementTree.parse(fileName)
        except (EnvironmentError, xls.parsers.expat.ExpatError, xls.etree.ElementTree.ParseError) as err:
    ...
    def exportar_db(self, *ignore):
        options = {}
        options['filetypes'] = [('Arquivos XLS', '.xls'), ('Todos Arquivos', '.*')]
        options['initialfile'] = 'PACIENTES.xls'
        ...
        try:
            with open(fileName, mode="w", encoding="UTF-8") as fh:
                fh.write('<?xls version="1.0" encoding="UTF-8"?>\n')
                fh.write("<pacientes>\n")
                for record in cursor:
                    fh.write('<pac sexo={0} cartao={1} '
                            ...
                            xls.sax.saxutils.quoteattr(record[1]),
                            xls.sax.saxutils.quoteattr(record[2]),
                            xls.sax.saxutils.quoteattr(record[3]),
                            xls.sax.saxutils.quoteattr(record[4]),
                            xls.sax.saxutils.quoteattr(record[5]),
                            xls.sax.saxutils.quoteattr(record[6]),
                            xls.sax.saxutils.quoteattr(record[7]),
                            xls.sax.saxutils.quoteattr(record[8]),
                            xls.sax.saxutils.quoteattr(record[9]),
                            xls.sax.saxutils.quoteattr(record[10]),
                            xls.sax.saxutils.quoteattr(record[11]),
                            xls.sax.saxutils.quoteattr(record[12])))
                    fh.write(xls.sax.saxutils.escape(record[0]))
                    fh.write("</pac>\n")
                fh.write("</pacientes>\n")
            messagebox.showinfo(title='Info', message='Arquivo XLS exportado com sucesso')
